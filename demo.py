...
def draw_graph(G, canvas_frame, highlight_edges_list=None):
    global pos
    ...
    if highlight_edges_list:
        colors = cycle(['red', 'green', 'purple', 'orange', 'brown', 'magenta'])
...
def find_shortest_path():
    ...
    if word2:
        try:
            all_paths = list(nx.all_shortest_paths(G, source=word1, target=word2, weight='weight'))
            result_lines = []
            highlight_paths = []
...
def calculate_pagerank(G, damping_factor, max_iter=100, tol=1e-6, word_list=None):
    N = len(G)
    if N == 0:
        return {}
    ...
    if word_list:
        ...
        total = sum(freq[node] for node in G if node in freq)
        pr = {node: freq[node] / total if node in freq else 0 for node in G}
    else:
    ...
    for _ in range(max_iter):
        new_pr = {}
        dangling_sum = sum(pr[node] for node in nodes if G.out_degree(node) == 0)
        ...
        for node in nodes:
            ...
            for pred in G.predecessors(node):
                if G.out_degree(pred) > 0:
                    rank += damping_factor * pr[pred] / G.out_degree(pred)
            new_pr[node] = rank
